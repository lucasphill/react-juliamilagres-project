...
export function Faq({ faq }: { faq: FaqItem[] }) {
  const items = [...faq].sort((a, b) => a.order - b.order).map((item) => ({ key: item.id, label: item.question, children: <p>{item.answer}</p>, forceRender: true, classNames: { body: 'ant-collapse-content-box' } }))
